...
class LoadStreams(Node):
    ...
    def __init__(self, sources='file.streams', img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        super().__init__('yolo_streams')
        torch.backends.cudnn.benchmark = True  # faster for fixed-size inference
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
        self.vid_stride = vid_stride  # video frame-rate stride
        sources = Path(sources).read_text().rsplit() if os.path.isfile(sources) else [sources] ## source가 파일이면 그 파일에서 source를 얻고 그렇지 않으면 직접 사용
        n = len(sources)
        self.sources = [clean_str(x) for x in sources]  # clean source names for later
        self.imgs, self.fps, self.frames, self.threads = [None] * n, [0] * n, [0] * n, [None] * n
        self.bridge = CvBridge()  # Add a CvBridge instance
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            s = eval(s) if s.isnumeric() else s  # i.e. s = '0' local webcam s = 0은 로컬 웹캠을 나타낸다. 그렇지 않으면 그대로 사용.
            if s == 0:
            ...
            self.fps[i] = fps  ## 이 source의 프레임 수와 fps를 설정한다.
            # _, self.imgs[i] = cap.read()  # guarantee first frame
            # self.threads[i] = Thread(target=self.update, args=([i]), daemon=True) ## 이 source에서 프레임을 계속 업데이트하기 위해 새 쓰레드를 시작한다.
            LOGGER.info(f'{st} Success ({self.frames[i]} frames {w}x{h} at {self.fps[i]:.2f} FPS)')
            # self.threads[i].start() ## 쓰레드 시작.
            while any(v is None for v in self.imgs):
                time.sleep(0.1)  # Wait for images to load
# 대기 시간을 줄여서 더 빠른 반응을 얻음
        while not all(t.is_alive() for t in self.threads):  # Wait until all threads are started
                    time.sleep(0.1)
        LOGGER.info('')  # newline
    ...
    def update(self, data):
        ...
        try:
            for i in range(len(self.sources)):
                self.imgs[i] = self.bridge.imgmsg_to_cv2(data, "bgr8")
                # height, width, _ = self.imgs[i].shape  # get the dimensions of the image
                # print("Image resolution: {}x{}".format(width, height))
                # print("self.imgs", self.imgs)
        except CvBridgeError as e:
            LOGGER.warning('WARNING ⚠️ CvBridgeError while converting image: ', e)
